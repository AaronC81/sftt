...
def fetch_tfl_routes(from_naptan, to_naptan, modes:, include_entrance_walking:)
  result = `curl \"https://api.tfl.gov.uk/Journey/JourneyResults/#{from_naptan}/to/#{to_naptan}?app_key=#{TFL_APP_KEY}&mode=#{modes}&routeBetweenEntrances=#{include_entrance_walking ? 'true' : 'false'}&date=20260322&time=1030\"`
    ...
    threads = [
      Thread.new do
        File.write(
          ...
          fetch_tfl_routes(from_naptan, to_naptan, modes: 'tube,walking', include_entrance_walking: false),
        )
      end,
      Thread.new do
        File.write(
          File.join(__dir__, 'routes', "#{from_crs}_#{to_crs}_any_with_transfers.json"),
          fetch_tfl_routes(from_naptan, to_naptan, modes: 'tube,walking', include_entrance_walking: true),
        )
      end,
      Thread.new do
        File.write(
